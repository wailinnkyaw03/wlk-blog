...
if(isset($_POST['search'])){
    $search = $_POST['search'];
    $allposts = $query->Search($search);
}else{
    $allposts = $query->getAll('posts', '*', 'INNER JOIN users on posts.created_by = users.id',null, "posts.created_at DESC");
}
?>
<div class="container">
    <div class="row">
        ...
        <?php else: ?>
                ...
                <div class="card my-3 shadow">
                    ...
                    <div class="body">
                        <div class="text px-3">
                            ...
                            <p class="mt-0 pt-0"><?= $allpost['description'] ?></p>
                        </div>
            ...
            <?php 
            if(count($allposts) == 0){ ?>
            <div class="text-center m-auto pb-3 mt-4">
                <h4>No Data Found</h4>
                <small class="d-block px-3">Sorry, but the page you are looking for has not been found. Try checking the URL error, then hit the refresh button on your browser or try found something else in our app.</small>
                <a href="index.php" class="btn btn-logo mt-4">Back To Home</a>
            </div>
            <?php } ?>
        <?php endif ?>
        </div>
